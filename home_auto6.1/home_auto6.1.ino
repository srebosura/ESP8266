...
else if ((request.indexOf("/1ON") != -1) && (digitalRead(SENS1_PIN) == 1)){ 
digitalWrite(DEV1_PIN, HIGH); } 
if (request.indexOf("/2OFF") != -1){ 
digitalWrite(DEV2_PIN, LOW); }
else if ((request.indexOf("/2ON") != -1) && (digitalRead(SENS2_PIN) == 1)){ 
...
client.println("</center>");
client.println("<center>");
client.println("<h1 style= \"background-color:LightSteelBlue\"><center><font face=\"Verdana\" size=\"20\" color=\"#E11711\"> Home Automation v6.1 </font></center></h2>");
client.println("</center>");
client.println("<center>");
...
client.println("<font face=\"Verdana\" size=\"12\" color=\"#00FF00\">");
client.println(mins);
client.println(":");
client.println(secs);
client.println("</font>");
...
if (timer_check_flag == 1)  {
  ...
  counter = round((millis() - timer_on)/1000);
    secs = counter%60;  
    mins = (counter/60)%60;
